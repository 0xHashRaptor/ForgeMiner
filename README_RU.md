...
### HiveOS
Custom miner — installation URL `.../ForgeMiner-<версия>.tar.gz`, шаблон кошелька `%WAL%.%WORKER_NAME%`, а в *Extra config* задайте `FORGE_ALGO=pearlhash` (или `qhash` / `kawpow` / `cryptix` / `btx`; для KawPow ещё `FORGE_COIN=rvn|quai|xna`). Готовые флайт-шиты: **[forgeminer.org/#flightsheets](https://forgeminer.org/#flightsheets)**.
...
- **Много монет, один бинарь** — Pearl, QubitCoin, KawPow (RVN / QUAI / XNA), Cryptix или BTX; выбор через `--algorithm`.
- **Ядра под архитектуру** — отдельное ядро под каждое поколение (Pascal / Turing / Ampere / Ada / Blackwell), выбирается при старте.
...
| Флаг | Env | Описание |
|------|-----|----------|
| `--algorithm` | `FORGE_ALGO` | `pearlhash`, `qhash`, `kawpow`, `cryptix` или `btx`. |
...
| **Pascal** | P104-100 · P106 · P108 (8 ГБ майнинг-карты) — Pearl, KawPow, Cryptix |
