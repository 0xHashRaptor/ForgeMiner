...
<p align="center"><b>Быстрый нативный майнер для NVIDIA под Pearl (PRL)</b></p>
...
**ForgeMiner** — высокопроизводительный полностью нативный майнер для блокчейна **Pearl (PRL)** (Proof-of-Useful-Work). Он работает с видеокартой напрямую через CUDA Driver API — **без Python, без WSL, без лишних рантаймов** — поэтому запускается мгновенно и потребляет минимум ресурсов даже на слабых ригах.

Ядро майнинга — вручную оптимизированный `int8`-движок на тензорных ядрах, созданный специально под Pearl NoisyGEMM + keyed-BLAKE3, с отдельным ядром под каждое поколение карт, чтобы каждый GPU работал на максимуме.
...
- 🌐 **Несколько пулов и диалектов** — из коробки работает со стандартными Pearl Stratum пулами (HeroMiners, LuckyPool) и с диалектом AlphaPool.
...
2. Откройте `.bat` своего пула (`HeroMiners.bat`, `LuckyPool.bat`, `AlphaPool.bat`) в текстовом редакторе и впишите свой **кошелёк** и **имя воркера**.
...
### ⛏️ HiveOS
Устанавливается как кастомный майнер — см. [`hiveos/README`](hiveos) в релизе. В полётном листе **имя майнера** — `ForgeMiner`; кошелёк/пул задаются в полётном листе как обычно.
...
| `--proto` | `FORGE_PROTO` | Диалект пула: `stratum` (HeroMiners, LuckyPool) или `alpha` (AlphaPool). |
...
В ForgeMiner встроена комиссия разработчика **3%**, которая майнится в фоне бесшовно, так что отображаемый хешрейт остаётся стабильным. Она идёт на развитие майнера и новые алгоритмы.
