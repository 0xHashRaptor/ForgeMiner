...
ForgeMiner — высокопроизводительный полностью нативный майнер для видеокарт NVIDIA. Он работает с видеокартой напрямую через CUDA Driver API — без Python, без WSL, без лишних рантаймов — поэтому запускается мгновенно и потребляет минимум ресурсов даже на слабых ригах. Первая поддерживаемая монета — Pearl (PRL), монета на Proof-of-Useful-Work; скоро будут и другие.
...
- **Ядра под архитектуру** — отдельное ядро под каждое поколение (Turing / Ampere / Ada / Blackwell), выбирается автоматически при запуске. Лидер по скорости на RTX 40-й серии и наравне с самыми быстрыми майнерами на RTX 50.
- **Эффективен на загруженных ригах** — держит карты «накормленными» даже при множестве карт на слабом CPU, нескольких инстансах или медленных x1-райзерах — там, где другие майнеры упираются в узкое место.
...
- **Управление по картам** — выбор карт для майнинга (`--gpu`) и разный разгон на каждую карту для смешанных ригов.
- **Несколько пулов и диалектов** — из коробки работает со стандартными Pearl Stratum пулами (BaikalMine, HeroMiners, LuckyPool, Kryptex) и с диалектом AlphaPool; автопереподключение и переключение между пулами.
- **Наглядная панель** — по каждой карте: хешрейт, принятые/устаревшие/отклонённые шары, эффективность (GH/W), температуры, частоты, вентиляторы и мощность.
...
### HiveOS
Создайте полётный лист Custom miner с такими полями:

| Поле | Значение |
|---|---|
| Имя майнера | `ForgeMiner` *(подставится из URL автоматически)* |
| Установочный URL | `https://github.com/0xHashRaptor/ForgeMiner/releases/download/v1.0.11/ForgeMiner-1.0.11.tar.gz` |
| Хэш-алгоритм | *(оставить пустым)* |
| Кошелёк и воркер (шаблон) | `%WAL%.%WORKER_NAME%` *(если привязан Pearl-кошелёк)* — или впишите явно `ВАШ_КОШЕЛЁК.%WORKER_NAME%` |
