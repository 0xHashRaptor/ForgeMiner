...
Ядро под Pearl — вручную оптимизированный `int8`-движок на тензорных ядрах под NoisyGEMM + keyed-BLAKE3, с отдельной сборкой под каждое поколение карт, чтобы каждый GPU работал на максимуме.
...
- 🌐 **Несколько пулов и диалектов** — из коробки работает со стандартными Pearl Stratum пулами (HeroMiners, LuckyPool, Kryptex) и с диалектом AlphaPool.
...
| **Установочный URL** | `https://github.com/0xHashRaptor/ForgeMiner/releases/download/v1.0.0/ForgeMiner-1.0.0.tar.gz` |
...
| **Адрес пула** | `45.151.62.119:3361` *(LuckyPool)* · `ru.pearl.herominers.com:1200` *(HeroMiners)* · `prl-ru.kryptex.network:7048` *(Kryptex)* |
| **Пароль** | `x` |
